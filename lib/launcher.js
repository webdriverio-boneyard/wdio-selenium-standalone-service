...
class SeleniumStandaloneLauncher {
    ...
    onPrepare (config) {
        ...
        return this._installSeleniumDependencies(this.seleniumInstallArgs).then(() => launcher._startSelenium(launcher.seleniumArgs))
    }
    ...
    _startSelenium (seleniumArgs) {
        return new Promise((resolve, reject) => Selenium.start(seleniumArgs, (err, process) => {
            if (err) {
                return reject(err)
            }
